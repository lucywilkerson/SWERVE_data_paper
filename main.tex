...
\authors{L. A. Wilkerson\affil{1}, R. S. Weigel\affil{1}, D. Bor\affil{1}, D. Thomas\affil{1}, C. T. Gaunt\affil{2}, P. J. Cilliers\affil{3}, E. J. Oughton\affil{1}}


\affiliation{1}{George Mason University}
\affiliation{2}{University of Cape Town}
\affiliation{3}{South Africa National Space Agency}
...
For example,  \citeA{Wilkerson2026} found that out of 396 GIC monitors with data openly available through the North American Electric Reliability Corporation (NERC), 355 had poor data quality for the May 2024 ``Gannon" storm. A further analysis of the NERC dataset by \citeA{Waghule2026} found that, for 29 GMD events occurring between 2015-2025, $\sim10\%$ of GIC sites had data of good quality for each event. As a result, ensuring the accessibility and usability of measured GIC data that is openly available is of interest to the research community.
...
There are two primary methods of observing GICs in a given power system: direct observation via Hall effect sensor and indirect observation via differential magnetometer measurement (DMM), also referred to as differential magnetometer method \cite{Parry2024}. Hall effect sensors take advantage of the low-frequency nature of GICs by measuring the DC component of the current flowing through the grounded neutral wire at a substation.
However, these sensors are expensive and difficult to maintain, and, as a result, their use globally is limited \cite{Hubert2020}. Examples of systems with operational Hall effect sensors include Alberta, Canada \cite{Parry2024}, New Zealand \cite{MacManus2025}, and Japan \cite{Watari2025}.


DMM was first developed as a way to quantify GICs in pipelines in Alaska \cite{Campbell1980}, but has since been adapted and applied to power networks as an indirect GIC observation method (\citeA{Makinen1993}; \citeA{Matandirotya2016}). The method uses two magnetometers, one at a remote site perpendicular to the power line that serves as a reference measurement for the variation of the background magnetic field and one placed directly under the power line to measure any excess current flow in addition to the variation of the background magnetic field. The difference between the two measurements yields the magnetic field associated with the GIC, from which the GIC through the line can be inferred using Ampère's Law or the Biot-Savart Law. While DMM is often not used by power grid operators due to the complications of deriving indirect measurements \cite{Marsal2021}, it is employed by researchers as DMM is cheaper and more flexible since it does not require direct access to substations \cite{Parry2024}.
...
The data collected from NERC includes GIC monitor and ground magnetometer data at NERC sites for 40 storms over North America. The selected storms include all storms through the end of March 2026 listed on NERC ERO portal. All storms after March 2026 are not due to be reported to NERC as of the time of this publication.
GMD data from both GIC monitors and ground magnetometers collected during a period from April 1 to March 31 is reported to NERC annually by June 30 \cite{NERC2026_reporting}; as such, released datasets from GMDs within the past year may not be complete and are not included in this paper.
